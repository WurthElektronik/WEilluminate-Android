    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        builder.setPositiveButton(R.string.deleteProfileDialogBtn, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                try {
                    Uri uri = Uri.parse(profile.getImagePath());
                    if (getActivity().getContentResolver().delete(uri, null, null) == 1) {
                        Log.d(CustomLogger.TAG, "Image deleted from " + profile.getImagePath());
                    } else {
                        Log.d(CustomLogger.TAG, "Error: Image could not be deleted!");
                    }
